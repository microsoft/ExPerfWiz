...
#################################################################################
#
# Script to help automate the collection of performance data on Exchange 2007/2010/2013/2016/2019 servers 
...
function checkUpdate
{
    ...
    if($script:version -ne $script:updateResult.Content)
    {
        ...
        if($script:updateResult.Content -ne $null)
        {
            ...
            if($answer -eq "yes")
            {
				Write-Debug "Adding BITS Transfer..."

				if(!(Test-Path -path "c:\ExPerfWiz-Downloads\")){
					Write-Debug "C:\ExPerfWiz-Downloads\ does not exist, creating it..."
					New-Item -ItemType Directory -Path "C:\ExPerfWiz-Downloads\" -Force | Out-Null
				}
				$versionToDownload = $script:updateResult.Content
				$pathofNewVersion = "C:\ExPerfWiz-Downloads\$($versionToDownload)\"
				$source = "https://github.com/Microsoft/ExPerfWiz/releases/download/$($versionToDownload)/ExPerfWiz.ps1"

				if(!(Test-Path $pathofNewVersion)){
				New-Item -ItemType Directory -Path $pathofNewVersion -Force | Out-Null
				}
...
function HealthCheckerScript
{
	Write-host "If you are experiencing performance issues, it is recommended you run the Exchange Healthchecker Script."
	Write-Host "Would you like to download it?" -NoNewline
	$answer = ConfirmAnswer
	if($answer -eq "yes"){
		$ie = new-object -comobject "InternetExplorer.Application"  
	    $ie.visible = $true  
		$ie.navigate("http://aka.ms/ExchangeHealthChecker")
		exit
	}
}
...
function GetExServerInfo
{
	...
	if ($Error)
	{
		...
		{
            Write-Debug "Exchange was not detected."
			Write-Host "================================================================"
			Write-Host ""
    ...
    elseif ($ExVersion -eq 15)
    {
        ...
        elseif ($ExVersionMinor -eq 2)
        {
            [bool]$script:Exchange2019 = $true
            Write-Debug "Exchange 2019 Detected"
        }
    }
}
...
function GetOSVersion
{
    ...
    $Script:OSVerBuild = ((Get-WmiObject Win32_OperatingSystem -ComputerName $ServerName).Version).Split(".")[2]
	If (($OSVerMajor -eq 5) -and ($OSVerMinor -eq 2)){
        $script:Windows2003 = $true
        Write-Debug "OS Version: Windows Server 2003 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
	elseif (($OSVerMajor -eq 6) -and ($OSVerMinor -eq 0)){
        $script:Windows2008 = $true
        Write-Debug "OS Version: Windows Server 2008 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
	elseif (($OSVerMajor -eq 6) -and ($OSVerMinor -eq 1)){
        $script:Windows2008R2 = $true
        Write-Debug "OS Version: Windows Server 2008 R2 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
    elseif (($OSVerMajor -eq 6) -and ($OSVerMinor -eq 2)){
        $script:Windows2012 = $true
        Write-Debug "OS Version: Windows Server 2012 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
    elseif (($OSVerMajor -eq 6) -and ($OSVerMinor -eq 3)){
        $script:Windows2012R2 = $true
        Write-Debug "OS Version: Windows Server 2012 R2 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
    elseif (($OSVerMajor -eq 10) -and ($OSVerMinor -eq 0) -and ($OSVerBuild -lt 17763)){
        $script:Windows2016 = $true
        Write-Debug "OS Version: Windows Server 2016 $OSVerMajor.$OSVerMinor.$OSVerBuild"
    }
    elseif (($OSVerMajor -eq 10) -and ($OSVerMinor -eq 0) -and ($OSVerBuild -ge 17763)){
        $script:Windows2019 = $true
        $regKey = "hklm:/software/microsoft/windows nt/currentversion"
        $Core = (Get-ItemProperty $regKey).InstallationType -eq "Server Core"
        if($Core){Write-Debug "OS Version: Windows Server 2019 Core $OSVerMajor.$OSVerMinor.$OSVerBuild"}
        Else{Write-Debug "OS Version: Windows Server 2019 $OSVerMajor.$OSVerMinor.$OSVerBuild"}
        
    }
    else{write-host "OS Version $OSVerMajor.$OSVerMinor.$OSVerBuild is not supported." -ForegroundColor Yellow;exit}
...
function CreateCounterList{
    ...
    if ($nofull -eq $true)
    {
        if ($Exchange2013 -or $Exchange2016 -or $Exchange2019)
        {
           write-host "Full collection is required for Exchange 2013/2016/2019. Enabling full collection."
           write-host
        }
        else
        {
...
elseif ($full -eq $true -and ($Exchange2013 -or $Exchange2016 -or $Exchange2019)){
	$Counters = @(
"\.NET CLR Exceptions(*)\*"
"\.NET CLR Memory(*)\*"
"\.NET CLR Loading(*)\*"
...
$script:roles += [string]"Full"
Write-Debug "Added Exchange 2013/2016/2019 Full Counters"
}
else{	
...
#	Check if Existing Exchange_Perfwiz Data Collection exists
	...
	$CheckifExists = Invoke-Expression -Command $QueryCollection
		...
		elseif ($Windows2008 -or $Windows2008R2 -or $Windows2012 -or $Windows2012R2 -or $Windows2016 -or $Windows2019){$SearchString = "Set was not found"}
...
function CreateCounter()
{
    ...
    if (($Exchange2013 -or $Exchange2016 -or $Exchange2019) -and ($maxsize -eq 512))
    {
        $maxsize = 1024
    }
	...
	if ($Windows2008 -or $Windows2008R2 -or $Windows2012 -or $Windows2012R2 -or $Windows2016 -or $Windows2019)
	{
		if ($circular)
		{
		...
		If ($Windows2008R2 -or $Windows2012 -or $Windows2012R2 -or $Windows2016 -or $Windows2019)
		{
			#Win2k8 R2 installed
	...
	if ($Windows2003 -or $Windows2008R2 -or $Windows2012 -or $Windows2012R2 -or $Windows2016 -or $Windows2019)
	{
	...
	#Add Duration
	if ($Windows2003 -or $Windows2008 -or $Windows2008R2 -or $Windows2012 -or $Windows2012R2 -or $Windows2016 -or $Windows2019)
	{
		if ($circular)
		{
...
{
    if ($Exchange2007 -or $Exchange2010 -or $Exchange2013 -or $Exchange2016 -or $Exchange2019)
    {
...
$regKey = "hklm:/software/microsoft/windows nt/currentversion"
$Core = (Get-ItemProperty $regKey).InstallationType -eq "Server Core"
if($Core){Write-Host "NOTE: The update check is disabled because the OS is Server Core" -ForegroundColor Yellow}

if((!$skipUpdateCheck) -and (!$Core) -and (!$start) -and (!$stop) -and (!$quiet) -and ($PSVersionTable.PSVersion.Major -ge 3)){checkUpdate}

if((!$Core) -and (!$Start) -and (!$stop) -and (!$quiet) -and ($PSVersionTable.PSVersion.Major -ge 3)){HealthCheckerScript}
