...
#################################################################################
#
# Script to help automate the collection of performance data on Exchange 2007/2010/2013 servers 
...
# 1.3.8 (Beta; brenle@microsoft.com)
...
function GetExServerInfo
{
    ...
    elseif ($ExVersion -eq 15){[bool]$script:Exchange2013 = $true}
...
function CreateCounterList{
    ...
    if ($nofull -eq $true)
    {
        if ($Exchange2013)
        {
           write-host "Full collection is required for Exchange 2013. Enabling full collection."
           write-host
        }
        else
        {
...
elseif ($full -eq $true -and $Exchange2013){
	$Counters = @(
"\.NET CLR Exceptions(*)\*"
"\.NET CLR Memory(*)\*"
"\.NET CLR Loading(*)\*"
...
$script:roles += [string]"Full"
Write-Debug "Added Exchange 2013 Full Counters"
}
else{	
...
function CreateCounter()
{
    ...
    #default to 1024 for E15
    if ($Exchange2013 -and ($maxsize -eq 512))
    {
        $maxsize = 1024
    }
...
{
    if ($Exchange2007 -or $Exchange2010)
    {
    ...
    {
        Write-Host "Exmon is not supported on Exchange 2013"
...
INFORMATION:
`tSets up Performance data collections for Exchange 2007/2010/2013 servers
...
`t-ExMon
`t`tAdds Exmon Tracing to specified server. Currently not supported
`t`twith Exchange 2013

`t-ExMonDuration
`t`tSets Exmon trace duration. If not specified, 30 minutes is the 
`t`tdefault duration.  Currently not supported
`t`twith Exchange 2013
